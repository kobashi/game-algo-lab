 ...
 * - 優先度 = g(n) のみ（ヒューリスティック h は使わない）
 * - 初期地図は js/maps/dijkstra-map.js をエディタで直接編集する
 */

import { parseMap } from "./map-format.js";
import { INITIAL_MAP } from "./maps/dijkstra-map.js";
import {
  setPanel,
  renderPriorityOpen,
  renderSet,
...
/** ダイクストラは h を使わない（表示用に 0 を置く） */
function heuristic(_x, _y) {
  return 0;
}

function updateHInfo() {
  if (hInfoEl) {
    hInfoEl.textContent = "優先度 = g（スタートからの経路コスト）。h は使わない";
    hInfoEl.classList.remove("h-info-warn");
    if (hasNegativeCost()) {
      hInfoEl.textContent += " ※負コストがあると最適保証が崩れます";
...
/** js/maps/dijkstra-map.js の INITIAL_MAP から読み込む */
function loadInitialMap() {
  const map = parseMap(INITIAL_MAP);
  if (map.cols !== COLS || map.rows !== ROWS) {
    throw new Error(
      `dijkstra-map.js のサイズは ${COLS}x${ROWS} にしてください（現在 ${map.cols}x${map.rows}）`
    );
  }
...
async function loadCsharpSample() {
  ...
  try {
    const res = await fetch("../samples/DijkstraExample.cs");
    if (!res.ok) throw new Error(String(res.status));
    const text = await res.text();
    csharpSample.textContent = text;
  } catch {
    csharpSample.textContent =
      "// samples/DijkstraExample.cs を読み込めませんでした。\n// ローカルサーバー経由で開いてください。";
  }
}
...
function reportResult(path) {
  ...
  if (optimal !== null && gGoal > optimal) {
    verdict = `⚠ 最適は g=${optimal} ですが、ダイクストラは g=${gGoal} でした。`;
    if (neg) {
      verdict += " 負コストがあると標準的なダイクストラの前提が崩れます。";
    }
  } else if (optimal !== null && gGoal === optimal) {
    verdict =
      "経路コスト g が最適と一致しました。コスト差のある地図では BFS（歩数最少）より有利になり得ます。";
  } else if (bfsCost !== null && gGoal < bfsCost) {
    verdict = `同一地図の BFS 経路コストは ${bfsCost}。ダイクストラはコストを見て有利な道を選べます。`;
  ...
  showCompare(`
    <h3>結果（ダイクストラ）</h3>
    <ul>
      <li><strong>経路コスト g(ゴール)</strong>: ${gGoal} ← 最小化の対象</li>
      <li><strong>真の最適（再訪なし）</strong>: ${optimal === null ? "—" : optimal}</li>
      <li><strong>歩数</strong>: ${hops}（副次的）</li>
      <li><strong>展開ノード数</strong>: ${expandCount}</li>
      <li><strong>優先度</strong>: g のみ（h = 0）</li>
    </ul>
    <p class="result-verdict">${verdict}</p>
    <p class="result-note">
      大きい数字 = <strong>g</strong>。A* の前段階として「実コストだけ」を見る探索です。
      ${neg ? "負コストがある場合は注意が必要です。" : ""}
    </p>
  `);
...
function stepOnce() {
  ...
  setStatus(
    `展開中… (${current.x},${current.y}) g=${curG} / open ${openSet.length}`
  );
  draw();
  updateDsViz();
  return true;
...
btnReset.addEventListener("click", () => {
  loadInitialMap();
  resetSearch();
  setStatus("dijkstra-map.js を再読み込みしました");
});
