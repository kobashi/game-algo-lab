...
| 最新試作 Release | https://github.com/kobashi/game-algo-lab/releases/tag/v0.17.0 |

**ready 110 トピック**。経路探索・ゲーム木・物理・空間・AI・設計・品質・HCI・プロシージャル・**サウンド**・**ゲーム CG**・**通信**まで一通り揃い、Sprint S5（影響マップ・Poisson・決定木）まで追加済みです。  
`rng-seed` / `tic-tac-toe` / `coyote-time` / `game-loop` / `velocity-motion` / `accel-gravity` / `gfx-ui-canvas` / `gfx-camera` / `input-basics` / `circle-collision` / `sfx-events` / `minimax` / `fsm` は URL クエリで初期パラメータを指定できます（授業課題の設定提出用）。
