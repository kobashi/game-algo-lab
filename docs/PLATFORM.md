...
| 適用対象 | `rng-seed` / `tic-tac-toe` / `coyote-time` / `game-loop` / `velocity-motion` / `accel-gravity` / `gfx-ui-canvas` / `gfx-camera` / `input-basics` / `circle-collision` / `sfx-events` / `minimax` / `fsm` |
...
- 再生中に DS が増えても **Map をウィンドウ追従で押し出さない**（新しい要素は上、疑似コードはパネル内スクロール）  
...
各ブロックはタイトルをクリックして **折りたたみ**（`<details>`）。開閉は `data-ds-key` でページ内に保持し、再生の再描画でも戻さない。

**再生中のスクロール**: 集合・リスト・親ポインタリスト・コールスタックは **新しい要素が上**（直近が先頭）。キューは FIFO のため左が先頭（取り出し）のまま、折り返さず横スクロール。疑似コードの行同期は **パネル内だけ** スクロールし、ウィンドウ（Map）は動かさない。
