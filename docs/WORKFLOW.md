...
### 2.1 merge 済みリモートブランチ（不要マーク）

クラウド環境などで作った作業ブランチが `main` に merge 済みでも、**すぐ削除せず残す**ことがある。  
その場合は `HANDOFF.md` の「リモートブランチ（不要マーク・削除しない）」表に記録する。

- **不要（保持）**: tip が `main` に包含済み。新規作業の起点にしない。削除はユーザー判断時のみ  
- 一覧の正は `HANDOFF.md`（エージェントはここを見て不要ブランチを作業先に選ばない）
...
## 10. マルチエージェント運用（Grok / Fable5 / Sonnet5）

複数の AI エージェントが同じリポジトリで協業する前提の役割分担。**役割は固定し、混在させない**。

| エージェント | 役割 | やること | やらないこと |
|--------------|------|----------|----------------|
| **Grok4.5** | 開発の起点 | 正本 docx の解釈、ROADMAP／CATALOG の企画立案、新規トピックの方向づけ | — |
| **Fable5** | レビュー専任 | 差分・ドキュメントの整合性チェック、指摘の提示（本レビューのような形） | **コード・ドキュメントの直接編集はしない**。指摘のみ |
| **Sonnet5**（本エージェント想定） | 実装 | Fable5 / Grok の指摘・企画を実コード・実ドキュメントへ反映、コミット | 独自の方針転換（正本の解釈変更）は行わない。判断が割れる点はユーザーに確認する |

### 引き継ぎの基本

1. レビュー（Fable5）は **指摘事項リスト**として残す（ファイル・行・矛盾点・根拠）。コードは触らない。
2. 実装（Sonnet5）は指摘を1件ずつ「対応／保留（理由付き）」で消化し、`HANDOFF.md` に反映内容を短く記録する。
3. 分類・方針レベルの矛盾（カテゴリ移動、正本の解釈違いなど）で **選択肢が複数あり優劣が明確でない場合は実装側で決め切らず、ユーザーに確認**してから反映する。
4. 正本 docx 自体の見直し（§4 領域表の抜けなど）は Grok 側の企画作業として扱い、Sonnet5 は ROADMAP.md 側の記述整合のみを直す（docx の書き換えはユーザー承認前提）。
